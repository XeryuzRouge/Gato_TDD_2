...
RSpec.describe PlayerConfiguration do   
  ...
  let(:player_configuration) { PlayerConfiguration.new(FakeInput) }
  ...
  it "should loop message error until it gets a valid character" do
    ...
    expect(output.messages[1]).to eq "h for human or c for computer.."
    expect(output.messages[2]).to eq "h for human or c for computer.."
  end
